...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/couperin_concerts.git
...
* `François Couperin – Concerts Royaux <https://github.com/DCMLab/couperin_concerts/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the first *Prélude* of the first concert has the following files:

-  ``MS3/c01n01_prelude.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/c01n01_prelude.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/c01n01_prelude.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/c01n01_prelude.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/c01n01_prelude.tsv')
   notes = ms3.load_tsv('notes/c01n01_prelude.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/couperin_concerts/issues>`__ and feel free to fork and submit pull requests.
